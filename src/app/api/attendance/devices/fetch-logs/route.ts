...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = parseLogs(logs, device.id, device.school_id);
    ...
    try {
      ...
      const [existing] = await connection.execute(
        `SELECT id FROM attendance_logs 
         ...
         LIMIT 1`,
        [log.school_id, log.device_id, log.device_log_id]
      );
      ...
      await connection.execute(
        `INSERT INTO attendance_logs 
         (school_id, device_id, device_user_id, scan_timestamp, received_timestamp, 
